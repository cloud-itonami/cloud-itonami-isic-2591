(ns metalforming.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean forging/pressing/
  stamping/roll-forming and powder-metallurgy plant through intake ->
  maintenance scheduling (escalate/approve) -> safety-concern flag
  (escalate/approve) -> shipment coordination (escalate/approve), then
  shows HARD-hold scenarios: a mis-wired request whose own `:effect`
  is not `:propose`, an unrecognized op, maintenance scheduled against
  an UNVERIFIED/unregistered equipment unit, a shipment coordinated
  ...
  that tries to ACTUATE the forging press or roll-forming mill
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on press-001 (verified, registered, forging press -- escalates, approve) ==")
    ...
    (let [r (exec-op actor "t3"
                      {:op :flag-safety-concern :effect :propose :subject "concern-1"
                       :value {:equipment-id "press-001" :severity :moderate
                               :description "鍛造プレス周辺で挟まれ点(ピンチポイント)ガードの緩みを確認"}}
                      coordinator)]
      (println r)
      (println "-- human plant supervisor approves --")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:product-category :forged-part}}
                       coordinator))
    ...
    (println "== schedule-maintenance mnt-2 on roll-002 (UNVERIFIED/unregistered roll-forming mill -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (600 kg would exceed weight 5000 vs shipped 4700 -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on press-001 with :actuate-forge-press-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-category :unobtainium-part}}
                       coordinator))
