(ns metalforming.governor
  "Metal Forming Plant Operations Governor -- the independent
  compliance layer that earns the MetalFormingAdvisor the right to
  ...
  draft-schedule) the forging press, stamping press, roll-forming mill
  or powder-metallurgy press-and-sinter line, whether a shipment
  ...
  proposal and becomes direct forging/pressing-line-equipment control,
  so this MUST be a separate system able to *reject* a proposal and
  fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:metal-forming-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:forge-press/
                                       actuate` or `:roll-mill/run`)
                                       is the 'direct forging/pressing-
                                       line-equipment control' scope
                                       violation this actor must NEVER
                                       perform -- HARD, PERMANENT,
                                       unconditional.
    4. Forge/press-line-actuate
       blocked                     -- for `:schedule-maintenance`, does
                                       the proposal's own `:value`
                                       declare `:actuate-forge-press-
                                       line? true`? Directly actuating
                                       the forging press, stamping
                                       press, roll-forming mill or
                                       powder-metallurgy press-and-
                                       sinter line is this actor's
    ...
    8. Shipment weight exceeded    -- for `:coordinate-shipment`,
                                       ...
                                       the batch's own recorded
                                       `:weight-kg`
                                       (`metalforming.registry/shipment-
...
(defn- forge-press-line-control-blocked-violations
  ...
  Anything else (direct forging-press/stamping-press/roll-forming-mill/
  powder-metallurgy-press-and-sinter-line control, a fabricated
  actuation effect) is this actor's central scope boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :forge-press-line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は鍛造プレス・スタンピングプレス等の直接操作に該当する可能性があり、恒久的に禁止")}]))

(defn- forge-press-line-actuate-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:actuate-forge-press-line? true` is
  attempting to directly actuate the forging press, stamping press,
  roll-forming mill or powder-metallurgy press-and-sinter line -- this
  actor may only ever propose/schedule a DRAFT maintenance window,
  never actuate the forging/pressing line directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-forge-press-line? (:value proposal))))
    [{:rule :forge-press-line-actuate-blocked
      :detail "鍛造プレス・スタンピングプレス・ロールフォーミングミル・粉末冶金焼結炉の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn check
  "Censors a MetalFormingAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (forge-press-line-control-blocked-violations proposal)
                           (forge-press-line-actuate-blocked-violations request proposal)
