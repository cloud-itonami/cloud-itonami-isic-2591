(ns metalforming.advisor
  "MetalFormingAdvisor -- the *contained intelligence node* for the
  forging/pressing/stamping/roll-forming and powder-metallurgy plant-
  operations coordination actor (ISIC 2591, forging, pressing,
  stamping and roll-forming of metal; powder metallurgy).

  It normalizes production-batch patches (product-category/weight/
  defect-rate), drafts a forging-press/stamping-press/roll-forming-
  ...
  and drafts an outbound forged/pressed/stamped/roll-formed/powder-
  metallurgy part shipment coordination proposal against a production
  batch. CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real forging/pressing-line actuation or
  freight dispatch. Every output is censored downstream by
  `metalforming.governor` before anything touches the SSoT -- see
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `metalforming.governor`)
  ...
  `metalforming.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [kotoba.lang.text :as str]
            [metalforming.registry :as registry]
            [metalforming.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch (forged/pressed/stamped/roll-formed/sintered lot)
  intake upsert -- the advisor only normalizes/validates the patch; it
  does not invent the batch's product-category, weight, defect-rate or
...
(defn- schedule-maintenance
  "Draft a forging-press/stamping-press/roll-forming-mill/powder-
  metallurgy-press-and-sinter-line maintenance-window scheduling
  ...
  `metalforming.governor` NEVER trusts this report -- it independently
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-forge-press-line?=" (boolean (:actuate-forge-press-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-forge-press-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a forging-press/stamping-press-pinch-crush / high-temperature-
  forging-burn-radiant-heat / roll-forming-mill-entanglement /
  powder-metallurgy-combustible-metal-dust-inhalation / equipment-
  safety concern. ALWAYS `:stake :coordination/safety-concern` -- a
  ...
  technicality). See `metalforming.phase`: no phase ever adds this op
  to a phase's `:auto` set; `metalforming.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound forged/pressed/stamped/roll-formed/powder-
  metallurgy part shipment coordination proposal against a production
  batch. The advisor passes through the caller's own claimed weight --
  it does NOT invent one, and `metalforming.governor` NEVER trusts it:
...
(def ^:private system-prompt
  (str "あなたは鍛造・プレス・スタンピング・ロールフォーミング"
       "および粉末冶金工場プラント運用コーディネーターの助言者です。"
       ...
       "鍛造プレス・スタンピングプレス・ロールフォーミングミル・粉末冶金焼結炉等の"
       "設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `metalforming.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :metalforming-advisor-proposal
   :op         (:op request)
