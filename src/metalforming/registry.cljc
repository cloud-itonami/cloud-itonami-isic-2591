(ns metalforming.registry
  "Pure-function domain logic for the forging/pressing/stamping/
  roll-forming and powder-metallurgy plant-operations coordination
  actor -- equipment/batch verification, shipment-weight recompute,
  product-category validation, defect-rate plausibility validation,
  ...
  pre-existing `kotoba-lang/metalforming`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `metalforming.governor` -- the same 'ground truth, not self-report'
  ...
  coordinated shipment), not the act of actuating a forging press,
  stamping press, roll-forming mill or powder-metallurgy press-and-
  sinter line, or dispatching a real freight carrier (this actor NEVER
  does either -- see README `What this actor does NOT do`).

  SCOPE NOTE: ISIC 2591 (this actor) is the forging/pressing/stamping/
  roll-forming and powder-metallurgy PROCESS shop -- typically a job
  shop/subcontractor that turns metal stock (billets, blanks, coil,
  metal powder) into near-net-shape forged/pressed/stamped/roll-formed/
  sintered parts for downstream manufacturers (including, but not
  limited to, the very cutlery/hand-tool/hardware shops of ISIC 2593
  and the other fabricated-metal-product shops of ISIC 2599). This is
  distinct from `cloud-itonami-isic-2592` (Treatment and coating of
  metals -- a distinct surface-finishing/heat-treatment service),
  `cloud-itonami-isic-2593` (Manufacture of cutlery, hand tools and
  general hardware -- the specific downstream finishing shop that
  forges/grinds/heat-treats/finishes stock into a named consumer
  product), and `cloud-itonami-isic-2599` (Manufacture of other
  fabricated metal products n.e.c. -- other specific downstream
  end-products not elsewhere classified). This actor's own hazard
  profile centers on forging-press/stamping-press pinch/crush hazard,
  high-temperature-forging burn/radiant-heat exposure, roll-forming-
  mill entanglement hazard, and powder-metallurgy combustible-metal-
  dust/inhalation exposure -- not sharp-edge cutlery/tool-edge
  laceration (2593) or coating/degreasing chemical exposure (2592).")

;; ----------------------------- constants -----------------------------

(def valid-product-categories
  "The closed set of product-category values a production-batch (a
  forged/pressed/stamped/roll-formed/sintered lot) record may declare
  -- the standard near-net-shape metal-forming output families this
  plant produces. Anything else is a fabricated/unrecognized product
  category -- the governor HARD-holds rather than let an invented
  category pass through."
  #{:forged-part :pressed-part :stamped-part :roll-formed-part
    :powder-metallurgy-part :forging-billet})

(def valid-process-types
  "The closed set of PRIMARY PROCESS types this plant's own production
  line may take -- forging, pressing, stamping, roll-forming, or
  powder metallurgy (press-and-sinter). A forging/pressing/stamping/
  roll-forming job shop never ships a finished consumer product (that
  is a different actor's own downstream/sibling scope, not this
  actor's)."
  #{:forging :pressing :stamping :roll-forming :powder-metallurgy})
...
(defn batch-registered?
  ...
  HARD invariant ('plant/batch record must be independently
  verified/registered before any action') exists to block."
  [batch]
...
(defn product-category-valid?
  ...
  values (forged/pressed/stamped/roll-formed/powder-metallurgy-part or
  forging-billet family)? nil/blank is treated as invalid (a
...
(defn register-maintenance
  ...
  forging-press/stamping-press/roll-forming-mill/powder-metallurgy-
  press-and-sinter-line maintenance window against a verified,
  registered piece of equipment. Pure function -- does not actuate the
  forging/pressing line or execute any maintenance; it builds the
  RECORD a plant coordinator would keep. `metalforming.governor`
  ...
  the forging/pressing line (see README `Actuation`), before this is
...
(defn register-shipment
  ...
  outbound forged/pressed/stamped/roll-formed/powder-metallurgy part
  shipment against a verified, registered production batch. Pure
  function -- does not dispatch any real freight carrier; it builds
  the RECORD a plant coordinator would keep. `metalforming.governor`
  independently re-verifies the shipment's own claimed weight against
