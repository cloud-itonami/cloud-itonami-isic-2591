(ns metalforming.store
  "SSoT for the forging/pressing/stamping/roll-forming and powder-
  metallurgy plant-operations coordination actor, behind a `Store`
  ...
  self-contained (no external forging/pressing/stamping/roll-forming
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             forged/pressed/stamped/roll-formed/
                             sintered lot)'s product-category/weight/
    ...
    - `equipment`         -- a forging-press/stamping-press/roll-
                             forming-mill/powder-metallurgy-press-and-
                             sinter unit's own record. `:verified?`/
    ...
    - `maintenance`       -- a scheduled forging/pressing/stamping/
                             roll-forming/powder-metallurgy-equipment
                             maintenance-window DRAFT against a piece
                             of equipment (`metalforming.registry`'s
    ...
    - `shipments`         -- a proposed outbound forged/pressed/
                             stamped/roll-formed/powder-metallurgy part
                             shipment DRAFT (`metalforming.registry`'s
                             `register-shipment`).
  ...
  coordinator needs."
  (:require [metalforming.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (metalforming.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (metalforming.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :forged-part :process-type :forging
                :material "Forged Crankshaft Billet Lot"
                :weight-kg 1800.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-weight-kg 300.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :stamped-part :process-type :stamping
                :material "Stamped Bracket Coil-Fed Lot"
                :weight-kg 5000.0 :defect-rate-percent 0.4
                :verified? true :registered? true
                :shipped-weight-kg 4700.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :powder-metallurgy-part :process-type :powder-metallurgy
                :material "Sintered Gear Blank Lot"
                :weight-kg 2200.0 :defect-rate-percent 1.1
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"press-001" {:id "press-001" :kind :forging-press
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "roll-002" {:id "roll-002" :kind :roll-forming-mill
               :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `metalforming.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `metalforming.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered forging-press unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  roll-forming-mill unit (blocks any maintenance scheduling against
