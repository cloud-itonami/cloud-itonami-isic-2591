(ns metalforming.operation
  "MetalFormingOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (MetalFormingAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Metal Forming Plant Operations Governor (:govern) and the rollout
  ...
  (:require [langgraph.graph :as g]
            ...
            [metalforming.advisor :as advisor]
            [metalforming.governor :as governor]
            [metalforming.phase :as phase]
...
(defn build
  "Compiles a MetalFormingOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `metalforming.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MetalFormingAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Metal Forming Plant Operations Governor -- independent censor.
      (g/add-node :govern
