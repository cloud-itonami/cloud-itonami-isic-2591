(ns metalforming.registry-test
  (:require [clojure.test :refer [deftest is]]
            [metalforming.registry :as r]))
...
(deftest small-shipment-within-weight-does-not-exceed
  (is (false? (r/shipment-weight-exceeded?
               {:weight-kg 1800.0 :shipped-weight-kg 300.0} 500.0))))

(deftest shipment-that-pushes-past-weight-exceeds
  (is (true? (r/shipment-weight-exceeded?
              {:weight-kg 5000.0 :shipped-weight-kg 4700.0} 600.0))))

(deftest shipment-exactly-at-weight-does-not-exceed
  (is (false? (r/shipment-weight-exceeded?
               {:weight-kg 5000.0 :shipped-weight-kg 4700.0} 300.0))
...
(deftest known-product-categories-are-valid
  (doseq [c [:forged-part :pressed-part :stamped-part
             :roll-formed-part :powder-metallurgy-part :forging-billet]]
    (is (r/product-category-valid? c))))

(deftest fabricated-product-category-is-invalid
  (is (not (r/product-category-valid? :unobtainium-part)))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "press-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "press-001" 1)
