(ns metalforming.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the forging press, stamping press,
  roll-forming mill or powder-metallurgy press-and-sinter line
  directly... does NOT authorize or execute maintenance/forge-press-
  line operations') implemented faithfully. The single invariant under
  test:

    MetalFormingAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Metal Forming Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :forged-part}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :forged-part (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest forge-press-line-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-forge-press-line? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:forge-press-line-actuate-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "press-001" :maintenance-type :die-inspection
                                          :scheduled-date "2026-08-01" :actuate-forge-press-line? false}} coordinator)
          _ (approve! actor "t9a")
...
(deftest invalid-product-category-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-category :unobtainium-part}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-category} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-part (:product-category (store/batch db "batch-001"))) "fabricated product-category never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "press-001" :severity :moderate
                                            :description "forging press pinch-point guard loose"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "press-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
