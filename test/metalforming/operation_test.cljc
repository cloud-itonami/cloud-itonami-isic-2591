(ns metalforming.operation-test
  "Smoke tests for the compiled MetalFormingOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `metalforming.governor-contract-test`; the Store contract in
  `metalforming.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MetalFormingOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "press-001" :severity :moderate :description "pinch-point guard loose near forging press"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
